...
class VRPDriver(NetworkDriver):
    ...
    def get_facts(self):
        ...
        vendor = u'Huawei'
        uptime = -1
        serial_number, fqdn, os_version, patch_version, hostname, model = (
            u'Unknown', u'Unknown', u'Unknown', u'Unknown', u'Unknown',
            u'Unknown')

        # obtain output from device
        show_ver = self.device.send_command('display version')
        show_patch = self.device.send_command('display patch')
        ...
        # os_version/patch_version/uptime/model
        PATCH_REGEXP = r"(?:Version\W+)(?P<patch_version>V\S+)"
        OS_REGEXP = r"\(S\S+\s+(?P<os_version>V\S+)\)"
        MODEL_REGEXP = r"S\S+"
        ...
        # patch_version
        for line in show_patch.splitlines():
            search_result = re.search(PATCH_REGEXP, line)
            if search_result is not None:
                patch_version = search_result.group('patch_version')
        ...
        serial_number = re.findall(SN_REGEXP, show_esn, flags=re.M)
        ...
        return {
            ...
            'patch_version': py23_compat.text_type(patch_version),
